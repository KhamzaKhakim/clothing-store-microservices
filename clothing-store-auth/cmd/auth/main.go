...
import (
	...
	"clothing-store-auth/internal/server"
...
func main() {
	startGrpc()
	connGrpc, err := grpc.Dial("localhost:8082", grpc.WithTransportCredentials(insecure.NewCredentials()))
	...
	client := pb.NewAuthServiceClient(connGrpc)
	//---------------------
...
func startGrpc() {
	...
	pb.RegisterAuthServiceServer(s, &server.Server{
		Logger: logger,
		Models: data.NewModels(db),
		Mailer: mailer.New(config.C.Smtp.Host, config.C.Smtp.Port, config.C.Smtp.Username, config.C.Smtp.Password, config.C.Smtp.Sender),
	})
